...
For example given options in an annotated method like this.
...
On default `help` for that command would show names coming
...
Define `spring.shell.option.naming.case-type=kebab` and
default modifier as added and option names then
look like.
