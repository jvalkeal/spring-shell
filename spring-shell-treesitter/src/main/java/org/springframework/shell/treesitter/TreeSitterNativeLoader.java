...
public class TreeSitterNativeLoader {
	...
	private static boolean mainLibLoaded = false;
	private static Map<String, Boolean> languageLibLoaded = new HashMap<>();
	...
	public static synchronized boolean initialize() {
        ...
        if (!mainLibLoaded) {
            Thread cleanup = new Thread(TreeSitterNativeLoader::cleanup, "cleanup");
            cleanup.setPriority(Thread.MIN_PRIORITY);
            cleanup.setDaemon(true);
            cleanup.start();
        }
        try {
            loadTreeSitterNativeLibrary(null);
        } catch (Exception e) {
            throw new RuntimeException("Unable to load treesitter native library: " + e.getMessage(), e);
        }
        return mainLibLoaded;
		// return true;
	}

	public static synchronized boolean initializeLanguage(String language) {
        // only cleanup before the first extract
        if (!languageLibLoaded.computeIfAbsent(language, l -> false)) {
            Thread cleanup = new Thread(TreeSitterNativeLoader::cleanup, "cleanup");
            cleanup.setPriority(Thread.MIN_PRIORITY);
            cleanup.setDaemon(true);
            cleanup.start();
        ...
        try {
            loadTreeSitterNativeLibrary(language);
        } catch (Exception e) {
            throw new RuntimeException("Unable to load treesitter native library: " + e.getMessage(), e);
        }
        return languageLibLoaded.computeIfAbsent(language, l -> false);
	}
	...
	private static File getTempDir() {
		return new File(System.getProperty("spring-shell.tmpdir", System.getProperty("java.io.tmpdir")));
	}
	...
	static void cleanup() {
		...
		File[] nativeLibFiles = dir.listFiles(new FilenameFilter() {
			private final String searchPattern = "treesitter-" + getVersion();
	...
	private static boolean extractAndLoadLibraryFile(String libFolderForCurrentOS, String libraryFileName,
		...
		try {
			...
			try (InputStream in = TreeSitterNativeLoader.class.getResourceAsStream(nativeLibraryFilePath)) {
			...
			try (InputStream nativeIn = TreeSitterNativeLoader.class.getResourceAsStream(nativeLibraryFilePath)) {
			...
			if (loadNativeLibrary(extractedLibFile)) {
				nativeLibrarySourceUrl = TreeSitterNativeLoader.class
	...
	private static boolean loadNativeLibrary(File libPath) {
		if (libPath.exists()) {
			...
			try {
				...
				log.warn("Failed to load native library:" + libPath.getName() + ". osinfo: "
						+ OSInfo.getNativeLibFolderPathForCurrentOS(), e);
				return false;
			}
	...
	private static void loadTreeSitterNativeLibrary(String language) throws Exception {
		...
		String jlineNativeLibraryPath = StringUtils.hasText(language)
				? System.getProperty("library.treesitter." + language + ".path")
				: System.getProperty("library.treesitter.path");
		String jlineNativeLibraryName = StringUtils.hasText(language)
				? System.getProperty("library.treesitter." + language + ".name")
				: System.getProperty("library.treesitter.name");
		if (jlineNativeLibraryName == null) {
			jlineNativeLibraryName = System.mapLibraryName(StringUtils.hasText(language) ? "tree-sitter-" + language : "tree-sitter");
		...
		if (jlineNativeLibraryPath != null) {
			...
			if (loadNativeLibrary(new File(withOs, jlineNativeLibraryName))) {
				if (StringUtils.hasText(language)) {
					languageLibLoaded.put(language, true);
				}
				else {
					mainLibLoaded = true;
				}
	...
	private static boolean hasResource(String path) {
		return TreeSitterNativeLoader.class.getResource(path) != null;
	}
	...
	public static String getVersion() {
		URL versionFile = TreeSitterNativeLoader.class.getResource("/META-INF/maven/org.jline/jline-native/pom.properties");
