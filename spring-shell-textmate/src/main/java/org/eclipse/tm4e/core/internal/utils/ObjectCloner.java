...
public final class ObjectCloner {
	...
	private static <T> T deepClone(final T obj, final Map<Object, Object> clones) {
		...
		// }
		if (obj instanceof final List<?> list) {
			List<Object> listClone = (List<Object>) shallowClone(list, () -> new ArrayList<>(list));
			clones.put(list, listClone);
			listClone.replaceAll(v -> deepCloneNullable(v, clones));
			return (T) listClone;
		}
		...
		// }
		if (obj instanceof final Map<?, ?> map) {
			Map<Object, Object> mapClone = (Map<Object, Object>) shallowClone(map, () -> new HashMap<>(map));
			clones.put(map, mapClone);
			mapClone.replaceAll((k, v) -> deepCloneNullable(v, clones));
			return (T) mapClone;
		}
	...
	private static <T> T deepCloneNullable(final T obj, final Map<Object, Object> clones) {
		if (obj == null) {
			return null;
		}
